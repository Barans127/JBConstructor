...
//        Resources.addImage("whiteSystemColor", "resources/ui/balta.png");
    ...
    public void setMaxShowableItems(int max){
        if (max <= 0){
            GdxPongy.getInstance().setError("PreparedRightClickMenu: max showable items cannot be <= 0", ErrorMenu.ErrorType.WrongPara);
            return;
        }
        maxItemsToShow = max;
    }
...
//        e.auto();
    ...
    public interface MeniuItemClickedListener{
        ...
        void clickedItem(Interface owner, int index, String itemName);
    }
    ...
    private class ButtonClick implements ClickListener{
        ...
        public void onClick() {
            if (listener != null) {
                int count = 0;
                for (Interface e : getController().getHost().getControls()) {
                    if (own == e){
