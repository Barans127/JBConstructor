...
public class Tip extends Balloon {
    ...
    public Tip(String text){
        this(text, Resources.getPropertyFloat("TipTextSize", 60), GdxPongy.getInstance().getFont(),
                Resources.getPropertyInt("TipTextColor", 0xFFFFFFFF), null, GdxPongy.getInstance().getScreenWidth(),
                GdxPongy.getInstance().getScreenHeight(), Align.left, Align.top);
    }
    ...
    public void show(Interface e){
        show(e, 0, 0);
    }

    public void show(Interface e, float offsetX, float offsetY){
        Vector2 point;
