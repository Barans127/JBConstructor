...
/** Leidžia vykdyt piešimą ar gaudyt inputus pirmiau nei formą. */
...
//		topsPosition.clear();
	...
	public boolean pinch(Vector2 initialPointer1, Vector2 initialPointer2, Vector2 pointer1, Vector2 pointer2) {
		for (int a = inputs.size()-1; a >= 0; a--){
			...
			Vector3 mouseCoords;
			int contrPos = -1;
			if (k instanceof Interface){
				contrPos = ((Interface) k).getController().getPositioning();
			}
...
//		mouseCoords.set(screenX, screenY, 0);
		...
		if (e instanceof Interface){
			contrPos = ((Interface) e).getAbstractPositioning();
		}
