...
import com.engine.interfaces.controls.ScrollField;
...
//            updateControlsLocation();
    ...
    public void removeAllItems(){
        ...
        for (int a = getHost().getControls().size() -1; a >= 0; a--){
            Interface f = getHost().getControls().get(a);
            if (f != e){
                removeControl(f);
            }
        }
    ...
    private void updateControlsSize(){
        List<Interface> list = getHost().getControls();
        for (Interface e : list){
            if (e instanceof Field){
                ((Field) e).setSize(itemWidth, itemHeight); // keiciam visu dydi i naujaji dydi.
            }else {
                GdxPongy.getInstance().setError("Interface without bounds found in listView. " +
...
//        int size = list.size() - 1; // reik ignoruot turnoff mygtuka....
        int size = 0;
        for (Interface e : list){ // bug fix kai padarydavo didesni dydi, nes nematomos kontroles budavo iskaiciuotos. Sitaip ir close mygtuka ignorina.
            if (e.isVisible()){
                size++;
            }
        }
        ...
        if (isVertical){
            ...
            for (Interface e : list){ // turn off mygtukas reik ignoruot....
                if (ignoreFirst){
                    ignoreFirst = false;
                    continue;
                }
