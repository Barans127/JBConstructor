...
import com.engine.interfaces.controls.widgets.Button.ButtonStyle;
...
public class TabControl extends Interface {
...
//		body.get(index).handle(); // nu ne tas. panelio reik..... ne pop up... ok.
	...
	@Override
	public void setController(InterfacesController e){
	...
	/* focus */
	/** Check if control is owned by tab control. Usually used by {@link InterfacesController} */
	public boolean containsControl(Interface e){
		...
		// o, gal panelio dalis?
		for (Interface in : body.get(index).getHost().getControls()){
			if (e == in){
				return true;
			}
		}
