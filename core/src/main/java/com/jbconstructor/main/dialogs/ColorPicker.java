...
import com.engine.interfaces.listeners.OnlyNumbers;
...
public class ColorPicker extends PopUp {
    ...
    public ColorPicker(){
        super(GdxPongy.getInstance().getScreenWidth()*0.7f, GdxPongy.getInstance().getScreenHeight()*0.83f);
//        Resources.addImage("colorDialogPicker", "resources/ui/colorPicker.png");
        ...
        if (picker == null){
            GdxPongy.getInstance().setError("Color picker: Default drawable was not found.", ErrorMenu.ErrorType.ControlsError);
            return;
        }
...
//        pickerMap = text.getTextureData().consumePixmap(); // pixmap, kad butu galima gaut reikiama pixelio spalva
        ...
        GdxPongy.addDisposable(pickerMap);
...
//    }
    ...
    public void changeSelectionImage(Drawable e){ // gal kada, kai prireiks.
        if (e != null){
            if (pickerMap != null) {
                GdxPongy.removeDisposable(pickerMap, true);
                pickerMap = null;
            }
