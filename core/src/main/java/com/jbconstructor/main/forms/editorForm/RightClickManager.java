...
import com.engine.interfaces.controls.widgets.ScrollBar;
...
import com.jbconstructor.main.dialogs.AnimationManager;
...
//        st.textColor = 0xffff4444;
    ...
    @Override
    protected boolean beforeRightClickOpen(Interface resizer, float x, float y, float offsetX, float offsetY) {
...
//            copy.setTextColor(0xff000000);
            disableItem(2, true);
            if (controlSize == 1) {
                Interface owner = e.getControls().get(0);
                ...
                for (int a = 0; a < e.getControls().size(); a++){
                    Interface face = e.getControls().get(a);
...
//                editForm.getResourcesDialog().open();
                ...
                if (size != 1){
                    ...
                    Interface in = e.getControls().get(0);
...
//                Resizer e = (Resizer) owner;
                if (e.getControls().size() > 0){
                    Interface face = e.getControls().get(0); // tik vienam.
...
//                Resizer e = (Resizer) owner;
                if (e.getControls().size() > 0){
                    Interface f = e.getControls().get(0);
...
//                Resizer e = (Resizer) owner;
                List<Interface> controls = e.getControls();
                if (controls.size() > 0) {
                    for (int a = 0; a < controls.size(); a++) {
                        Interface f = controls.get(a);
                        if (f instanceof Element) {
...
//                Resizer e = (Resizer) owner;
                for (int a = 0; a < e.getControls().size(); a++) {
                    Interface f = e.getControls().get(a);
                    if (f instanceof Element) {
                        Element resource = (Element) f;
                        resource.lock(lock);
                    }
