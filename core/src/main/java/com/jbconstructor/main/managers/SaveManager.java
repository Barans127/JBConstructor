...
//            ChainEdging chainManager = form.getMainPanel().getChainEdgePanel().getChainEdgingManager();
            ...
            MAINLOOP:
            for (Interface control : selectiveInterfaceController.getControls()) {
                ...
                for (Interface ex : selectiveInterfaceController.getExceptionList()) { // ignoruojamos kontroles.
                    if (control == ex) {
...
//                    }
            ...
            for (int a = 0; a < manager.getFormsSize(); a++){
                ...
                if (!export) { // nereik kameros ir settings. juos tik constructorius naudoja.
                    ...
                    if (GdxPongy.getInstance().getActiveForm() != e) {
                        ...
                        OrthographicCamera cam = GdxPongy.getInstance().getAbsoluteCamera();
                ...
                MAINLOOP:
                for (Interface c : contr.getControls()){
                    if (c instanceof Resizer || c instanceof Selector){
                        continue; // ignoruojam situos.
                    }
                    for (Interface ex : contr.getExceptionList()){
                        if (c == ex){
                            continue MAINLOOP; // exeption liste, reiks neleist.
                        }
                    }
