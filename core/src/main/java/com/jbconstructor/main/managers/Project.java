...
//    }
    ...
    public static void closeProject(){
        if (GdxPongy.getInstance().getActiveForm() instanceof EditForm){ // reikia perjungt i StartForma.
            GdxPongy.getInstance().achangeState("mainStartForm"); // perjunks i start forma, kad nemaisytu istrintos editformos.
        }
...
//                Gdx.app.log("Project", "Cannot start deletion chain with null values.");
    ...
    private static void checkResourceDeletionAvailability(final ResourceToHold[] holds, final Array<String> atlases){
        ...
        for (int a = 0; a < formsManager.getFormsSize(); a++){ // visos esancios formos.
            ...
            List<Interface> array = form.getController().getControls(); // formoje esantys elementai.
            for (Interface e : array){
...
//                this.shapes.addAll(shapes);
        ...
        void delete(){
            ...
            for (int a = 0; a < formsManager.getFormsSize(); a++){
                ...
                List<Interface> array = form.getController().getControls();
                for (Interface e : array){
