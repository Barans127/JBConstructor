...
//                visibleElements.add(e);
    ...
    private void setAllItemVisible(Array<ElementItem> elementItems){
        ...
        List<Control> list = host.getControls();
        if (list instanceof ArrayList){
            ((ArrayList<Control>) list).ensureCapacity(active.size);
        }
    ...
    protected void onShow() {
        ...
        for (Control in : resizer.getControls()) {
...
//        visibleElements.clear();
        ...
        // einam per items.
        List<Control> exceptionList = controller.getExceptionList();
        int index = 0;
...
//    }
    ...
    private class ElementItem extends SymbolButton implements Connected, Pool.Poolable {
        private Control item; // musu laikomas item.
