...
import com.jbconstructor.editor.dialogs.ResourcesDialog;
...
//                Project.getSaveManager().wakeWorker(); // kad susigaudytu kas daros, pradetu auto save daryt.
        ...
        sizer.setChangeListener(new Resizer.ChangeListener() {
            @Override
            public void onSizeChange(Control e, float x, float y, float width, float height) { } // nope

            @Override
            public void onPositionChange(Control e, float x, float y) {} // nope
...
//        sizer.setRightClickListener(rightClickManager);
    ...
    public void checkControlInfo() {
        if (sizer.hasMultipleControls()) {
            ...
            Control e = sizer.getControls().get(0);
...
//    }
    ...
    public void paste(){
        Array<Control> copies = copyManager.paste();
...
//                            checkResourceDrawable(e); // tiesiog pazes ar ne spriter drawable.
    ...
    public boolean deleteSelectedItems(){
        if (sizer.getControls().size() > 0){
            ...
            for (Control e : sizer.getControls()){ // trinam visas kontroles is formos.
