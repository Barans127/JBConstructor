...
import com.jbconstructor.editor.dialogs.FormCreateDialog;
...
public class StartForm extends Form {
    ...
    private Control selectedItem; // kad lengviau remove mygtuka valdyt butu.
...
//        lwst.multiSelect = true;
        ...
        list.setActiveItemClickListener(new PreparedListView.ActiveItemClickListener() {
            @Override
            public void onActiveItemClick(Control activeItem, int index, Object userData) {
                if (index >= 0 && index < filesLocation.size) {
                    chosenPath = filesLocation.get(index); // ???? index out of bounds exception????
                }else {
                    GdxWrapper.getInstance().setError("List has more items. It should be: " + filesLocation.size, ErrorMenu.ErrorType.ControlsError);
                    return;
                }
...
//        errorMessage.setTime(10000);
    ...
    private boolean create(String name, String rootLocation, String folderPath){
        if (Project.getFormsManager().getFormsSize() > 0){ // fatal. kazkokia fujne nutiko.
            GdxWrapper.getInstance().setError("Cannot create new Project. Existing project is opened.", ErrorMenu.ErrorType.ControlsError);
            return true;
        }
        ProjectLoader e = new ProjectLoader();
...
//        con.open();
    ...
    protected void background() {
        GdxWrapper.getInstance().background(0xFFeaef83); // kazkas i geltona.
    }
