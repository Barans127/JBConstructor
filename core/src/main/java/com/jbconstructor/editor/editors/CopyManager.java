...
public class CopyManager {
    private Array<Control.InterfaceStyle> copies;
    private Array<Control> tmpInterfaceList;
    ...
    public void copy(List<Control> list){
        // perorderinimui reiks situ masyvu.
        Control[] interfaces = new Control[list.size()];
        ...
        if (window instanceof EditForm){
            ...
            List<Control> controls = form.getController().getControls();
            for (int a = 0; a < controls.size(); a++){
                Control e = controls.get(a);
                for (int k = 0; k < list.size(); k++){
                    Control compare = list.get(k);
                    if (e == compare){
                        indexes[k] = a;
                    }
                }
            ...
            while (currentIndex < interfaces.length){
                Control selected = null; // dabartinis pasirinkimas.
        ...
        for (Control e : interfaces){
            Control.InterfaceStyle st = e.getStyle();
    ...
    public Array<Control> paste(){
        tmpInterfaceList.clear();
        for (Control.InterfaceStyle e : copies){
    ...
    public Array<Control> getCopies(){
        return tmpInterfaceList;
    }
