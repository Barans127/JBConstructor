...
import com.engine.ui.controls.dialogs.InputTextDialog;
...
import com.jbconstructor.editor.managers.FormsManager;
...
//    private SymbolButton.SymbolButtonStyle style; // kad butu lengva pridet formas.
    ...
    public SwitchFormsDialog(final String formName) {
        super(GdxWrapper.getInstance().getScreenWidth()*0.9f, GdxWrapper.getInstance().getScreenHeight()*0.6f);
//        this.owner = owner;
...
//        ((AlertToast) error).setTextSize(60);
    ...
    private  void updateEditItemsLocation(){
        Control e = formsTabs.getHost().getControls().get(lastIndex+1);
    ...
    private void activeItemClick(Control e, int index){
        if (index == lastIndex){
            formsTabs.deselectItems();
            open.setVisible(false);
            delete.setVisible(false);
    ...
    private void checkFormsChange(){
        ...
        for (int a = 0, b = 1; a < formsSize; a++, b++){
            if (a == size-2){ // trukst dalies.
                ...
                Control e = formsTabs.getHost().getControls().get(b);
